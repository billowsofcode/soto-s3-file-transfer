...
extension S3FileTransferManager {
    ...
    public func copy(from: String, to: S3File, options: PutOptions = .init(), progress: @escaping (Double) throws -> Void = { _ in }) async throws {
        self.logger.info("Copy from: \(from) to \(to)")
        let eventLoop = self.s3.eventLoopGroup.next()
        let fileHandle: NIOFileHandle
        let fileRegion: FileRegion
        do {
            (fileHandle, fileRegion) = try await self.fileIO.openFile(path: from, eventLoop: eventLoop).get()
        } catch {
        ...
        do {
            ...
            if fileSize > self.configuration.multipartThreshold {
                ...
                _ = try await self.s3.putObject(request, logger: logger, on: eventLoop)
                try? progress(1.0)
            }
