...
final class S3FileTransferManagerAsyncTests: XCTestCase {
    ...
    func testUploadDownload() {
        ...
        XCTRunAsyncAndBlock {
            try await Self.s3FileTransfer.copy(from: filename, to: S3File(bucket: Self.bucketName, key: "testFile"), options: .init(metadata: ["test": "1,2,3"])) { print($0) }
            //try Self.s3FileTransfer.copy(from: S3File(bucket: Self.bucketName, key: "testFile"), to: filename2) { print($0) }
